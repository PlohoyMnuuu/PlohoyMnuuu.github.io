...
function renderPromocodes(codes) {
    ...
    if (!codes || codes.length === 0) {
        c.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text-dim);">⚠️ Промокоды временно недоступны. Обновите страницу позже.</div>';
        updatePromoCount();
        return;
    }
    ...
    c.innerHTML = codes.map(function(item, i) {
        ...
        return '<div class="promo-card" onclick="copy(\'' + safeCode + '\')" role="button" tabindex="0">' +
            '<div class="promo-info">' +
                '<div class="code-header">' +
                    '<span class="code-text">' + item.code + '</span>' + 
                    badge + 
                '</div>' +
                '<span class="reward-text">' + (item.reward || 'Бонус') + '</span>' +
            '</div>' +
            '<div class="copy-btn">' +
                '<svg fill="none" viewBox="0 0 24 24">' +
                    '<rect height="16" rx="2" stroke="#000" stroke-width="2" width="12" x="8" y="2"/>' +
                    '<path d="M4 6V20H16" stroke="#000" stroke-linecap="round" stroke-width="2"/>' +
                '</svg>' +
                'Копировать' +
            '</div>' +
        '</div>';
...
function loadPromocodes() {
    ...
    fetch('https://script.google.com/macros/s/AKfycbxu_zefZP7nxFfGR1aLN5QtiFTGk4qFhMR6EGh3Of330wlkszzRkjAd_dig7Ww22bdb/exec')
        ...
        .then(function(data) {
            console.log('Получено промокодов:', data.length);
            if (data && data.length > 0) {
                try { 
                    localStorage.setItem('promo_cache', JSON.stringify(data)); 
                } catch(e) {}
...
function showErrorPlaceholder() {
    ...
    if (c) {
        c.innerHTML = '<div style="text-align:center;padding:40px;">' +
            ...
            '<div style="color:var(--text-dim);">Не удалось загрузить промокоды</div>' +
            '<button onclick="loadPromocodes()" style="margin-top:16px;padding:8px 24px;background:var(--accent);border:none;border-radius:12px;cursor:pointer;">Обновить</button>' +
            '</div>';
    }
    updatePromoCount();
}
